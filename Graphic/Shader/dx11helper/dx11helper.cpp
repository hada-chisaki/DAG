...
　DDSファイルを読み込みシェーダーリソースビュー―を作成する
 --------------------------------------------------------------*/
bool CreateSRVfromDDS_2(const char* filename,
	ID3D11Device* device,
	ID3D11DeviceContext* device11Context,
...
　ＴＧＡファイルを読み込みシェーダーリソースビュー―を作成する
 --------------------------------------------------------------*/
bool CreateSRVfromTGAFile_2(const char* filename,
	ID3D11Device* device,
	ID3D11Resource** resource,
	ID3D11ShaderResourceView** srv) {
...
　WICファイルを読み込みシェーダーリソースビューを作成する
 --------------------------------------------------------------*/
bool CreateSRVfromWICFile_2(const char* filename,
	ID3D11Device* device,
	ID3D11DeviceContext* device11Context,
	ID3D11Resource** resource,
	ID3D11ShaderResourceView** srv) {
...
bool CreateSRVfromFile_2(
	...
	// DDSファイル
	if (ext == "dds") {
		bool sts = CreateSRVfromDDS_2(fname.c_str(),
			device,
			device11Context,
			resource,
			srv);
	...
	// TGAファイル
		...
		bool sts = CreateSRVfromTGAFile_2(fname.c_str(),
			device,
			resource,
			srv);
		if (!sts) {
	...
	// その他ファイル
		...
		bool sts = CreateSRVfromWICFile_2(fname.c_str(),
			device,
			device11Context,
			resource,
			srv);
